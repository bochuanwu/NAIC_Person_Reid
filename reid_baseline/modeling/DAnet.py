...
class Baseline(nn.Module):
    ...
    def __init__(self, num_classes, last_stride, model_path, neck, neck_feat, model_name, pretrain_choice,with_ibn,gcb,stage_with_gcb):
        ...
        #PCM CAM
        self.head = DANetHead(2048, num_classes, nn.BatchNorm2d)
    ...
    def forward(self, x):
        attention_feat = self.gap(self.head(self.base(x)))
        ...
        if self.neck == 'no':
            ...
            feat2 = self.bottleneck2(attention_feat)

        if self.training:
            cls_score = self.classifier(feat)
            cls_score2 = self.classifier2(feat2)
            return [cls_score,cls_score2], [global_feat,attention_feat]  # global feature for triplet loss
        else:
            if self.neck_feat == 'after':
                # print("Test with feature after BN")
                return torch.cat((feat,feat2),1)
            else:
...
class DANetHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer):
        ...
        self.conv52 = nn.Sequential(nn.Conv2d(inter_channels, inter_channels, 3, padding=1, bias=False),
                                    norm_layer(inter_channels),
                                    nn.ReLU())
        self.conv53 = nn.Sequential(nn.Conv2d(inter_channels, 2048, 3, padding=1, bias=False),
                                    norm_layer(2048),
                                    nn.ReLU())
